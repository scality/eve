...
class EveMaster(object):
    ...
    def deploy(self, master_docker_host):
        """Deploy an EVE instance on docker."""
        logger.info('=> Creating docker image <%s> on %s...' % \
                    (self.master_fqdn, master_docker_host))
        self.docker = Docker(
            self.master_fqdn,
            docker_host=master_docker_host,
            docker_cert_path='certs/docker_master')
        self.docker.build_image(
            ...
            worker_cert_path='certs/docker_workers'
        )
        logger.info('=> Removing docker instance of <%s> if it exists' %\
                    self.master_fqdn)
        self.docker.rm_all(force=True)
        logger.info('=> Creating a new instance of <%s>' % self.master_fqdn)
        container_id = self.docker.run(self.master_fqdn, env_vars=self.eve_env_vars)
        return container_id
...
def main():
    ...
    parser.add_argument('git_repo_owner',
                        help='The git repo owner. e.g., scality')
    parser.add_argument('git_repo_name',
                        help='The git repo name. e.g., wall-e, ring')
    ...
    eve = EveMaster(
        bitbucket_git_repo='git@git@bitbucket.org:%s/%s' % (
            args.git_repo_owner,
            args.git_repo_name),
        master_fqdn='%s.eve.devsca.com' % args.git_repo_name,
        worker_docker_host=os.environ['WORKERS_DOCKER_HOST']
    )
    ...
    container_id = eve.deploy(
        master_docker_host=os.environ['MASTER_DOCKER_HOST'],
    )
